...
const CreateRecipeForm = ({ createRecipeHandler, recipe, step }) => {
    const [poster, setPoster] = useState({ src: recipe.imageUrl, file: null })
    const [youtubeUrl, setYoutubeUrl] = useState({
        value: recipe?.videoUrl,
        isTouched: false,
        isValid: true,
    })
    const [categories, setCategories] = useState(recipe.listCategories)
    const [method, setMethod] = useState(recipe.cookingMethodId)
    const [uses, setUses] = useState(recipe.usesId)
    const [continents, setContinents] = useState(recipe.recipeRegionId)
    const [name, setName] = useState({ value: recipe?.name, isTouched: false })
    const [description, setDescription] = useState({ value: recipe?.description, isTouched: false })
    const [toolsPreparation, setToolsPreparation] = useState({
        value: step?.tool,
        isTouched: false,
    })
    const [ingredientsPreparation, setIngredientsPreparation] = useState({
        value: step?.ingredient,
        isTouched: false,
    })
    const [process, setProcess] = useState({ value: step?.processing, isTouched: false })
    const [cook, setCook] = useState({ value: step?.cooking, isTouched: false })
    const { routerPrompt, setFormIsTouched } = usePrompt('Changes you made may not be saved.')
    const [servingPeople, setServingPeople] = useState(step.serving)
    const [preparingTime, setPreparingTime] = useState(step.preparingTime)
    const [processingTime, setProcessingTime] = useState(step.processingTime)
    const [cookingTime, setCookingTime] = useState(step.cookingTime)
